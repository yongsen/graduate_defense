...
\begin{block}{802.11n的PHY/MAC新技术使得网络测试与配置更为复杂}
传统的无线网络配置或基于主动探测，或基于固定PDR-RSS模型，在802.11n网络中无法满足多种配置及高传输速率的要求。
如图\ref{MCS}所示，在所有被选择的HT/GI/MCS配置中，大约34\%落入过渡窗口中，甚至有8\%的配置使得PDR$<$10\%，这种情况在移动802.11n网络中更为明显。
\end{block}
\begin{figure}[!t]
\setcounter{subfigure}{0}
\centerline{
    \subfloat[]{\includegraphics[width=0.42\textwidth]{modeling1.pdf}
    \label{modeling1}
}
    \subfloat[]{\includegraphics[width=0.27\textwidth]{pdr.pdf}
    \label{pdr}
}
    \subfloat[]{\includegraphics[width=0.25\textwidth]{MCS.pdf}
    \label{MCS}
}
}
\end{figure}
